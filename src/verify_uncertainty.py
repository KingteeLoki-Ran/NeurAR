...
def render(H, W, K, chunk=1024*32, rays=None, c2w=None, ndc=True,
    ...
    k_extract = ['rgb_map', 'disp_map', 'acc_map', 'uncertainty_map']
...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        print(i, time.time() - t)
        t = time.time()
        rgb, disp, acc, uncer, _ = render(H, W, K, chunk=chunk, c2w=c2w[:3,:4], **render_kwargs)
...
def render_rays(ray_batch,
    ...
    Returns:
      ...
      uncertainty_map: [num_rays]. uncertainty of each ray
      raw: [num_rays, num_samples, 4]. Raw predictions from model.
      rgb0: See rgb_map. Output for coarse model.
      disp0: See disp_map. Output for coarse model.
      acc0: See acc_map. Output for coarse model.
      uncertainty_map0: See uncertainty_map. Output for coarse model
...
#     raw = run_network(pts)
    ...
    if N_importance > 0:
        ...
        rgb_map_0, disp_map_0, acc_map_0, uncertainty_map0 = rgb_map, disp_map, acc_map, uncertainty_map
...
#         raw = run_network(pts, fn=run_fn)
        ...
        uncertainty_map = raw[...,4]
        rgb_map, disp_map, acc_map, weights, depth_map = raw2outputs(raw, z_vals, rays_d, raw_noise_std, white_bkgd, pytest=pytest)
    ret = {'rgb_map' : rgb_map, 'disp_map' : disp_map, 'acc_map' : acc_map, 'uncertainty_map' : uncertainty_map}
    if retraw:
        ret['raw'] = raw
    if N_importance > 0:
        ret['rgb0'] = rgb_map_0
...
def train(args):
    ...
    if args.dataset_type == 'llff':
        ...
        i_train = np.delete(i_train,[4,6,13,15,17,19,20,22,25,28,30,33,36,43,49,50,59,63,68,72,73,74,77,80,86,95,99])
        near = 2.
        far = 6.
    ...
    elif args.dataset_type == 'unity':
        images, depths, poses, render_poses, hwf, i_split = load_unity_data(args.datadir, args.half_res, args.testskip)
        logging.info('Loaded blender image shape is ' + str(images.shape) + " render_poses shape is " + str(render_poses.shape)+ " hwf are " + str(hwf) + " args.datadir  " + str(args.datadir))
        print('Loaded blender', images.shape, render_poses.shape, hwf, args.datadir)
        i_train, i_val, i_test = i_split
        i_train = i_train[:int(len(i_train)*3/4)]
    ...
    elif args.dataset_type == 'unityl':
        images, depths, poses, render_poses, hwf, i_split = load_unity_alexander(args.datadir, args.half_res, args.testskip)
        logging.info('Loaded blender image shape is ' + str(images.shape) + " render_poses shape is " + str(render_poses.shape)+ " hwf are " + str(hwf) + " args.datadir  " + str(args.datadir))
        print('Loaded blender', images.shape, render_poses.shape, hwf, args.datadir)
        i_train, i_val, i_test = i_split
        i_train = i_train[:int(len(i_train)*3/4)]
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            logging.info('test poses shape', render_poses.shape)

            rgbs_u, _ , uncer  = render_path(render_poses, hwf, K, args.chunk, render_kwargs_test, gt_imgs=images, savedir=testsavedir, render_factor=args.render_factor)
            logging.info('Done rendering', testsavedir)
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            ...
            logging.info("batch_rays, target_S :" + str(batch_rays.shape) + " , " +  str(target_s.shape))
            logging.debug("batch_rays, target_S :" + str(batch_rays[0]) + " , " +  str(target_s[0]))
        ...
        rgb, disp, acc, uncertainty, extras = render(H, W, K, chunk=args.chunk, rays=batch_rays,
        ...
        img_loss_perray = img2mesrespective(rgb, target_s)
        img_loss = torch.sum(img_loss_perray)

        delta = torch.sum(uncertainty)

        loss = torch.log(delta) + img_loss/(delta ** 2)
        
        psnr = mse2psnr(img2msepsnr(rgb,target_s))
        ...
        if 'rgb0' in extras:
            ...
            img_loss_perray0 = img2mesrespective(extras['rgb0'], target_s)
            ...
            loss = loss + loss0
            psnr0 = mse2psnr(img2msepsnr(extras['rgb0'], target_s))
        ...
        if i%args.i_video==0 and i > 0:
            ...
            with torch.no_grad():
                rgbs_u, disps, uncer = render_path(render_poses, hwf, K, args.chunk, render_kwargs_test)
        ...
        if i%args.i_uncertainty==0 and i > 0:
            ...
            with torch.no_grad():
                rgbs_u , __, uncer = render_path(poses[index], hwf, K, args.chunk, render_kwargs_test, gt_imgs=images[index], savedir=testsavedir)
                rgbs_u = torch.from_numpy(rgbs_u).to(device)
                uncer = torch.Tensor(uncer).to(device)
                
                for img_index in range(0,len(rgbs_u)):
                    img_loss_perray_un = img2mesrespective(rgbs_u[img_index], torch.from_numpy(images[index[img_index]]).to(device))
                    img_loss_un = torch.sum(img_loss_perray_un)
                    delta_un = torch.sum(uncer[img_index])
                    loss_un = torch.log(delta_un) + img_loss_un/(delta_un ** 2)
                    psnr_un = mse2psnr(img2msepsnr(rgbs_u[img_index], torch.from_numpy(images[index[img_index]]).to(device)))
...
if __name__=='__main__':
    ...
    logging.basicConfig(level=logging.INFO,
                    filename=f'../logs/{args.expname}.log',
                    filemode='a',
                    format=
